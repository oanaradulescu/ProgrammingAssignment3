best <- function(state, condition) {
        ...
        lowest.mr <- as.character(df$"Mortality.Rate"[1])
        df <- df[ !is.na(df$"Mortality.Rate") & df$"Mortality.Rate" == lowest.mr, ]
        df <- df[ order(as.character(df[,"Hospital.Name"]), df[,"State"], as.numeric(as.character(df[,"Mortality.Rate"]))), ]
